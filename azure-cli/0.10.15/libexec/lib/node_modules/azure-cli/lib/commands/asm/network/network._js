...
exports.init = function (cli) {
  ...
  appGateway.command('create [name] [vnet-name] [subnet-name]')
    ...
    .option('-t, --subnet-name <subnet-name>', $('the name of subnet which exists in same vnet'))
    ...
    .option('-s, --subscription <subscription>', $('the subscription identifier'))
    .execute(function (name, vnetName, subnetName, options, _) {
      ...
      options.subnetName = cli.interaction.promptIfNotGiven($('Subnet name: '), subnetName, _);

      var networkManagementClient = createNetworkManagementClient(options);
      var appGateway = new AppGateway(cli, networkManagementClient);
      appGateway.create(name, options, _);
    });

  appGateway.command('set [name]')
