        ...
        fs.readFile(options[propFrom], function(err, data) {
          if (!err) {
            ...
            if (propTo === 'publicConfig') {
              ...
              config['chef_daemon_interval'] = options.chefDaemonInterval;
              if (options.referenceName == 'ChefClient' && options.daemon) {
                if (['none', 'service', 'task'].indexOf(options.daemon) != -1) {
                  ...
                  return callback(
                    new Error($('Invalid user input for --daemon option.')));
                }
              } else if (options.daemon) {
                return callback(
                    new Error($('The daemon option is valid for Windows node.')));
              }

              if (options.jsonAttributes) {
                try {
                  config['custom_json_attr'] = JSON.parse(options.jsonAttributes);
                } catch (er) {
              ...
              if (options.secretFile) {
                config['encrypted_data_bag_secret'] = fs.readFileSync(options.secretFile).toString();
                options[propTo] = JSON.stringify(config);
              } else if (options.secret) {
                config['encrypted_data_bag_secret'] = options.secret;
                options[propTo] = JSON.stringify(config);
              }
            }
          }
...
function createVMView(role, deployment, logger) {
  ...
  try {
    ...
    result = {
      ...
      MaintenanceStatus: !roleInstance.maintenanceStatus ? {} : {
        IsCustomerInitiatedMaintenanceAllowed: (roleInstance.maintenanceStatus.isCustomerInitiatedMaintenanceAllowed),
        PreMaintenanceWindowStartTime: (roleInstance.maintenanceStatus.preMaintenanceWindowStartTime ? roleInstance.maintenanceStatus.preMaintenanceWindowStartTime : {}),
        PreMaintenanceWindowEndTime: (roleInstance.maintenanceStatus.preMaintenanceWindowEndTime ? roleInstance.maintenanceStatus.preMaintenanceWindowEndTime : {}),
        MaintenanceWindowStartTime: (roleInstance.maintenanceStatus.maintenanceWindowStartTime ? roleInstance.maintenanceStatus.maintenanceWindowStartTime : {}),
        MaintenanceWindowEndTime: (roleInstance.maintenanceStatus.maintenanceWindowEndTime ? roleInstance.maintenanceStatus.maintenanceWindowEndTime : {}),
        LastOperationResultCode: (roleInstance.maintenanceStatus.lastOperationResultCode ? roleInstance.maintenanceStatus.lastOperationResultCode : {}),
        LastOperationMessage: (roleInstance.maintenanceStatus.lastOperationMessage ? roleInstance.maintenanceStatus.lastOperationMessage : {})
      },
